...
class Condition
{
   public:
    ...
    /// its own Mutex.
    Condition() : m_(new Mutex()), own_mutex_(true) { pthread_cond_init(&cv_, NULL); }
    ...
    Condition(Mutex* m) : m_(m), own_mutex_(false) { pthread_cond_init(&cv_, NULL); }
    ~Condition()
    {
        if (own_mutex_) delete m_;
    }
...
#define WAIT_WHILE(a)                               \
    m_->Lock();                                     \
    while (a) pthread_cond_wait(&cv_, &m_->mutex_); \
    m_->Unlock()

    inline void WaitWhileTrue(bool* b) { WAIT_WHILE(*b); }
    inline void WaitWhileFalse(bool* b) { WAIT_WHILE(!*b); }
    template <typename T>
    inline void WaitWhileNull(T** p)
    {
        WAIT_WHILE(*p == NULL);
    }
    template <typename T>
    inline void WaitWhileNonNull(T** p)
    {
        WAIT_WHILE(*p != NULL);
    }
    template <typename T>
    inline void WaitWhileEq(const T& exp, T* act)
    {
        WAIT_WHILE(*act == exp);
    }
    template <typename T>
    inline void WaitWhileGt(const T& exp, T* act)
    {
        WAIT_WHILE(*act > exp);
    }
    template <typename T>
    inline void WaitWhileGe(const T& exp, T* act)
    {
        WAIT_WHILE(*act >= exp);
    }
    template <typename T>
    inline void WaitWhileLt(const T& exp, T* act)
    {
        WAIT_WHILE(*act < exp);
    }
    template <typename T>
    inline void WaitWhileLe(const T& exp, T* act)
    {
        WAIT_WHILE(*act <= exp);
    }
...
#define SIGNAL_IF(a)                  \
    ...
    template <typename T>
    inline bool SignalIfNull(T* p)
    {
        SIGNAL_IF(*p == NULL);
    }
    template <typename T>
    inline bool SignalIfNonNull(T* p)
    {
        SIGNAL_IF(*p != NULL);
    }
    template <typename T>
    inline bool SignalIfEq(const T& exp, T* act)
    {
        SIGNAL_IF(*act == exp);
    }
    template <typename T>
    inline bool SignalIfGt(const T& exp, T* act)
    {
        SIGNAL_IF(*act > exp);
    }
    template <typename T>
    inline bool SignalIfGe(const T& exp, T* act)
    {
        SIGNAL_IF(*act >= exp);
    }
    template <typename T>
    inline bool SignalIfLt(const T& exp, T* act)
    {
        SIGNAL_IF(*act < exp);
    }
    template <typename T>
    inline bool SignalIfLe(const T& exp, T* act)
    {
        SIGNAL_IF(*act <= exp);
    }
