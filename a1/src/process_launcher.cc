...
ProcessLauncher::ProcessLauncher(int max_outstanding) : Launcher()
{
    ...
    *max_outstanding_ = max_outstanding;
    ...
    /*
     * Map done_mutex_ and done_cond_ into a memory segment that is shared
     ...
     * Note that the call to mmap only _allocates_ memory for done_mutex_
     * and done_cond_. We still need to intialize them appropriately.
     */
    done_mutex_ = (pthread_mutex_t *)mmap(NULL, sizeof(pthread_mutex_t), PROT_FLAGS, MAP_FLAGS, 0, 0);
    pthread_mutexattr_t mutexattr;
    pthread_mutexattr_init(&mutexattr);
    pthread_mutexattr_setpshared(&mutexattr, PTHREAD_PROCESS_SHARED);
    pthread_mutex_init(done_mutex_, &mutexattr);

    done_cond_ = (pthread_cond_t *)mmap(NULL, sizeof(pthread_cond_t), PROT_FLAGS, MAP_FLAGS, 0, 0);
    pthread_condattr_t condattr;
    pthread_condattr_init(&condattr);
    pthread_condattr_setpshared(&condattr, PTHREAD_PROCESS_SHARED);
    pthread_cond_init(done_cond_, &condattr);
}
...
ProcessLauncher::~ProcessLauncher()
{
    ...
    err = munmap((void *)done_mutex_, sizeof(pthread_mutex_t));
    assert(err == 0);

    pthread_cond_destroy(done_cond_);
    err = munmap((void *)done_cond_, sizeof(pthread_cond_t));
    assert(err == 0);
}
...
void ProcessLauncher::ExecuteRequest(Request *req)
{
    ...
    /*
     * Track the number of requests issued. launcher::ExecuteRequest is
    ...
    { /* This code is executed in the parent */
        ...
        {
            pthread_cond_wait(done_cond_, done_mutex_);
        }
        --*max_outstanding_;
        pthread_mutex_unlock(done_mutex_);
    }
